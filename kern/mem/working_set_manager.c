...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
    ...
    //Your Code is Here...
//	cprintf("In create \n");
//	cprintf("virtual_address:%x\n",virtual_address);
    struct WorkingSetElement* newWSElement;
    newWSElement = (struct WorkingSetElement*)kmalloc(sizeof(struct WorkingSetElement));
//    cprintf("newElement:%x\n",newWSElement);
    if (newWSElement == NULL)
    {
//    	cprintf("could not map.\n");
       panic("env_page_ws_list_create_element: Failed to allocate memory for WorkingSetElement");
    }
    newWSElement->virtual_address = ROUNDDOWN(virtual_address,PAGE_SIZE);
//    cprintf("newElement va:%x\n",newWSElement->virtual_address);
    newWSElement->empty = 0;
    newWSElement->time_stamp = 0;
