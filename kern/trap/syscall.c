...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size)
{
	...
	char* ptt =(char*)virtual_address;
	if (ptt == NULL)
	{
		env_exit();
	}
	if(virtual_address>USER_HEAP_MAX || virtual_address<USER_HEAP_START ||(virtual_address+size)<USER_HEAP_START ||(virtual_address+size)>USER_HEAP_MAX)
	{
		env_exit();
	}
...
uint32 sys_get_alloc_size(uint32 virtual_address )
{
	char* ptt =(char*)virtual_address;
	if (ptt == NULL)
	{
	...
	if(virtual_address>USER_HEAP_MAX || virtual_address<USER_HEAP_START)
	{
		env_exit();
	}
	return get_alloc_size(cur_env, virtual_address);
}
//2014
...
void sys_semaphore_operation(struct Env_Queue* queue,uint32* locks, int op_num)
{
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk_y :
	        ...
	        break;
	case SYS_free_user_mem_y :
		sys_free_user_mem(a1,a2);
		return 0;
		break;
	case SYS_allocate_user_mem_y:
		sys_allocate_user_mem(a1,a2);
		return 0;
		break;
	case SYS_env_set_priority_y:
		sys_env_set_priority((int32)a1,(int)a2);
		return 0;
		break;
	case SYS_get_alloc_size_y:
		return sys_get_alloc_size(a1);
		break;
	case SYS_semaphore_operation_y:
		sys_semaphore_operation((struct Env_Queue*)a1,(uint32*)a2,(int)a3);
		return 0;
		break;
	//======================================================================
	case SYS_cputs:
