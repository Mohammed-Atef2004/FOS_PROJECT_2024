...
struct Env* fos_scheduler_PRIRR()
{
	...
	if (cur_env != NULL)
	{
		sched_insert_ready((struct Env *)cur_env);
	}
...
void clock_interrupt_handler(struct Trapframe* tf)
{
	if (isSchedMethodPRIRR())
	{
		...
		struct Env* itr_env = NULL;
		int curr_size = 0 ;
		for(int i = 1;i<num_of_ready_queues;i++)
		{
			curr_size = queue_size(&ProcessQueues.env_ready_queues[i]);
			if(curr_size != 0)
			{
				LIST_FOREACH(itr_env, &(ProcessQueues.env_ready_queues[i]))
				{
					itr_env->starvation_number++;
					if (itr_env->starvation_number > Starv_threshold)
					{
						itr_env->starvation_number = 0;
						itr_env->priority--;
						sched_remove_ready(itr_env);
						sched_insert_ready(itr_env);
					}
				}
			}
		}
