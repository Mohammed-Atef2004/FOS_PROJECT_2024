...
int matched(const char *sourceCommand, const char *targetCommand){
    unsigned int targetCommandLength = strlen(targetCommand);
    unsigned int j = 0;
    for(int i = 0; sourceCommand[i] != '\0' && j< targetCommandLength; i++ )
    {
        if(sourceCommand[i] == targetCommand[j])
        {
            j++;
        }
    }
    return (j == targetCommandLength);
}
...
int process_command(int number_of_arguments, char** arguments)
{
    ...
    for (int i = 0; i < NUM_OF_COMMANDS; i++)
    {
        if (strcmp(arguments[0], commands[i].name) == 0)
        {
            ...
            if((commands[i].num_of_args == -1 && (number_of_arguments - 1) > 0) || (number_of_arguments - 1) == commands[i].num_of_args){
                return i;
            }
    ...
    for(unsigned int i = 0; i < NUM_OF_COMMANDS; i++)
    {
        if(matched(commands[i].name, arguments[0]))
        {
            LIST_INSERT_HEAD(&foundCommands, &commands[i]);
        }
    }
    int listSize = LIST_SIZE(&foundCommands);
    if(listSize > 0)
    {
        return CMD_MATCHED;
    }
