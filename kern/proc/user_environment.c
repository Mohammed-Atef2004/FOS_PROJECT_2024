...
#if USE_KHEAP
	{
	struct WorkingSetElement* env_wse;
	LIST_FOREACH(env_wse,&e->page_WS_list){
		if(env_wse->prev_next_info.le_next!=0){
		env_page_ws_invalidate(e, env_wse->virtual_address);
		}
		else{
			env_page_ws_invalidate(e, env_wse->virtual_address);
			break;
...
# else
	{
		if (!isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)){
			for(int i=0;i<__PWS_MAX_SIZE;i++){
...
# endif
	// [2] remove working set
#if USE_KHEAP
	{
      // clear all values of working set list
	if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)){
...
# else
	 {
		if (!isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)){
			...
			e->ptr_pageWorkingSet=0;
		}


}
# endif
	// [3] delete user_kern_stack
...
#if USE_KHEAP
		{
			ptable = (uint32*) kheap_virtual_address(pa);
		}
#else
		{
			ptable = (uint32*) STATIC_KERNEL_VIRTUAL_ADDRESS(pa);
		}
#endif
...
#else
		{
			decrement_references(to_frame_info(e->env_cr3));
		}
#endif
...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
    ...
    void* stack_base = kmalloc(KERNEL_STACK_SIZE);
        if (stack_base == NULL) {
            panic("create_user_kern_stack: Failed to allocate memory for user kernel stack");
        }
...
//	panic("delete_user_kern_stack() is not implemented yet...!!");
	kfree(e->kstack);
	int numOfPages= ROUNDUP(KERNEL_STACK_SIZE,PAGE_SIZE)/PAGE_SIZE;
	uint32 stackpva=(uint32)e->kstack;
	for(int i=0;i<numOfPages;i++){
		unmap_frame(e->env_page_directory,stackpva);
		stackpva+=PAGE_SIZE;
	}
	e->kstack=0;
	e->initNumStackPages=0;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
    ...
    e->uhhard_limit=(uint32*)daLimit;
    e->uhsegmant_break=(uint32*)daStart;
